...
class Process114(Process):
    ...
    key=lambda x : x[0]
        ...
        date_data = dataframe_2["EXECUTION_DATE"][0].strftime('%Y%m%d')
        caratula=self.create_caratula(list_of_fields, date_data)
    ...
    def create_caratula(self, list_of_fields, date_data):
        ...
        run=p.add_run()
        run.add_break()
        p.add_run(f'Información correspondiente a la fecha: {date_data}                               Archivo: {self._codigo_reporte}')
