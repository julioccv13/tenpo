DROP TABLE IF EXISTS `tenpo-bi.tmp.permanencia_usuario_suscripciones{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.permanencia_usuario_suscripciones{{ds_nodash}}` AS 
(
...
variables as (
    SELECT
        ...
        ARRAY<STRING>['Suscripcion'] as products
),
cohort_fecha AS (
    SELECT id AS user,
           cohort,
...
users_maus AS (
    ...
    FROM `tenpo-bi-prod.economics.economics` a,
         variables
    WHERE
        nombre IN UNNEST(variables.products)
...
tabla_final_agrup as (
  ...
  case
    ...
    when cohort='mau_mes_ob'         then 'MAU_MES_OB'
...
select distinct user as identity, cohort as permanencia_usuario_suscripciones
