DROP TABLE IF EXISTS `tenpo-bi.tmp.ventana_pdc{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.ventana_pdc{{ds_nodash}}` AS 
(

select distinct b.user as identity,
CASE WHEN a.value IS NOT NULL THEN CAST(a.value AS STRING) ELSE 'NULL' END as prom_30dias_ventana_pdc
...
where c.codename = 'ventana30_pdc'
