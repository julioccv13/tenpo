DROP TABLE IF EXISTS `tenpo-bi.tmp.recencia_suscripciones_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.recencia_suscripciones_{{ds_nodash}}` AS 
(
...
CASE when a.value = 0 then 'N/A' when a.value IS NOT NULL THEN CAST(a.value AS STRING) ELSE 'N/A' END as recencia_suscripciones_d
...
where c.codename = 'recencia_suscripciones_d'
)

select a.user as identity, CASE WHEN b.identity is not null then b.recencia_suscripciones_d else 'N/A' end as recencia_suscripciones_d
from users_ob a 
