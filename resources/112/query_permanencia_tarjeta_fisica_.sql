DROP TABLE IF EXISTS `tenpo-bi.tmp.permanencia_tarjeta_fisica_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.permanencia_tarjeta_fisica_{{ds_nodash}}` AS 
(
...
variables as (
    SELECT
        ...
        ARRAY<STRING>['mastercard_physical'] as products
),
cohort_fecha AS (
    SELECT id AS user,
           cohort,
...
select distinct user as identity, cohort as permanencia_usuario_tarjeta_fisica_
