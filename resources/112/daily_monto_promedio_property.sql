DROP TABLE IF EXISTS `${project_target}.tmp.daily_monto_promedio_property_{{ds_nodash}}`;
CREATE TABLE `${project_target}.tmp.daily_monto_promedio_property_{{ds_nodash}}` AS (
WITH
TRANSACCIONES AS 
(
  select
  ...
  and user not in (select distinct id from `tenpo-airflow-prod.users.users` where EXTRACT(date from ob_completed_at) >= DATE_ADD(CURRENT_DATE(), INTERVAL -3 MONTH))
...
select distinct user as identity, case when CLASIFICACION_CLIENTE = 'Cliente Monto OK' then 1 else 0 END clasificacion_monto_promedio_cliente_daily
