DROP TABLE IF EXISTS `tenpo-bi.tmp.frecuencia90_suscripciones_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.frecuencia90_suscripciones_{{ds_nodash}}` AS 
(
...
CASE when a.value = 0 then 'N/A' when a.value IS NOT NULL THEN CAST(a.value AS STRING) ELSE 'N/A' END as frecuencia90_suscripciones
...
where c.codename = 'frecuencia90_suscripciones'
)

select a.user as identity, CASE WHEN b.identity is not null then b.frecuencia90_suscripciones else 'N/A' end as frecuencia90_suscripciones
from users_ob a 
