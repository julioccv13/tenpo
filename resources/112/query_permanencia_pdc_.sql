DROP TABLE IF EXISTS `tenpo-bi.tmp.permanencia_usuario_pdc_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.permanencia_usuario_pdc_{{ds_nodash}}` AS 
(
...
variables as (
    SELECT
        ...
        ARRAY<STRING>['utility_payments'] as products
),
cohort_fecha AS (
    SELECT id AS user,
           cohort,
...
select distinct user as identity, cohort as permanencia_usuario_pdc_
