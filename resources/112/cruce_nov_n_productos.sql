DROP TABLE IF EXISTS `tenpo-bi.tmp.cruce_nov_n_productos_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.cruce_nov_n_productos_{{ds_nodash}}` AS (
...
select a.identity,
  CASE 
  WHEN b.identity is not null then b.cruce_nov_n_productos else -1 end as cruce_nov_n_productos
from users_ob a 
