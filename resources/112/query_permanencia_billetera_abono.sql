DROP TABLE IF EXISTS `tenpo-bi.tmp.permanencia_usuario_billetera_abono_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.permanencia_usuario_billetera_abono_{{ds_nodash}}` AS 
(
...
variables as (
    SELECT
        ...
        ARRAY<STRING>['paypal_abonos'] as products
),
cohort_fecha AS (
    SELECT id AS user,
           cohort,
...
select distinct user as identity, cohort as permanencia_usuario_billetera_abono
