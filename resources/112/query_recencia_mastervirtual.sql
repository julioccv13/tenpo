DROP TABLE IF EXISTS `tenpo-bi.tmp.recencia_mastercard_virtual{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.recencia_mastercard_virtual{{ds_nodash}}` AS 
(
...
CASE when a.value = 0 then 'N/A' when a.value IS NOT NULL THEN CAST(a.value AS STRING) ELSE 'N/A' END as recencia_mastercard_virtual
...
where c.codename = 'recencia_master_d' 

)

select a.user as identity, CASE WHEN b.identity is not null then b.recencia_mastercard_virtual else 'N/A' end as recencia_mastercard_virtual
from users_ob a 
