DROP TABLE IF EXISTS `tenpo-bi.tmp.frecuencia90_pdc_{{ds_nodash}}`;
CREATE TABLE `tenpo-bi.tmp.frecuencia90_pdc_{{ds_nodash}}` AS 
(
...
CASE when a.value = 0 then 'N/A' when a.value IS NOT NULL THEN CAST(a.value AS STRING) ELSE 'N/A' END as frecuencia90_pdc
...
where c.codename = 'frecuencia90_pdc'
)

select a.user as identity, CASE WHEN b.identity is not null then b.frecuencia90_pdc else 'N/A' end as frecuencia90_pdc
from users_ob a 
