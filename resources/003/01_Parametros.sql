...
DROP TABLE IF EXISTS `{{project_id}}.temp.TPT_{{ds_nodash}}_003_Params`;
CREATE TABLE `{{project_id}}.temp.TPT_{{ds_nodash}}_003_Params` AS (
    WITH airflow_params AS (
        ...
        SELECT
            Fecha_Ejec
            ,DATETIME_TRUNC(Fecha_Ejec, DAY) AS Fecha_Inicio_Ejec
            ,DATETIME_SUB(DATETIME_TRUNC(Fecha_Ejec, DAY), INTERVAL 1 DAY) AS Fecha_Inicio_Analisis
            ,DATETIME_SUB(DATETIME_TRUNC(Fecha_Ejec, DAY), INTERVAL 1 SECOND) AS Fecha_Fin_Analisis
