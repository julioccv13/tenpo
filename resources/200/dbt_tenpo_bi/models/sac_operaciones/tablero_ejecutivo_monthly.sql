{{ 
    config(
        ...
        ,alias='tablero_ejecutivo_monthly'
    )
}}

with table1 as (
select DATE_TRUNC(dia, MONTH) as fecha_mes,
ejecutivo,
...
group by 1, 2 )
select fecha_mes, ejecutivo, ROUND(numerador_csat/denominador_csat,3) as csat_mes, ROUND(numerador_fcr/denominador_fcr,3) as fcr_mes,
