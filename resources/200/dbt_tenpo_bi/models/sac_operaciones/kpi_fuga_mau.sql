...
),  -- usuarios con ticket y son mau en mes x, Ademas no son MAU en mes 2
    user_kp1 as (
        select A.user, A.fecha_month from TICKET_MAUS_mes1 A
        LEFT JOIN MAUS B on A.user=B.user and A.fecha_month_2=B.fecha_month
        WHERE B.user IS NULL
)

select fecha_month, count(*) as cantidad from user_kp1
