...
select 
  ...
  count(distinct IF(tip_trx="RETIRO_APP",id_cuenta,null)) over (partition by date_trunc(date(fec_creacion_cuenta),month)) as cant_cuenta_mensual_app,
  fec_ult_trx_exitosa,
  cant_trx,
  dias_prom_entre_trx,
...
from clientes 
  ...
  join categorias USING (id_cuenta)