...
SELECT 
    DISTINCT
      ...
      utilities.name cupon_merchant, -- comercio que gatila el cupon
      utility_payments.amount cupon_spent, -- gasto en cupon
...
LEFT JOIN {{source('tenpo_utility_payment','bills')}} utility_payments on utility_payments.id = redeems.id_trx
LEFT JOIN {{source('tenpo_utility_payment','utilities')}} utilities on utilities.id = utility_payments.utility_id
WHERE transaction_type LIKE 'PAY_BILL%'
