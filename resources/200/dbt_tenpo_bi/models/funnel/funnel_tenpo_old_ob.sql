...
* EXCEPT(ID), 
ID 
FROM {{source('tenpo_users','onboarding_PROD')}} --`tenpo-airflow-prod.users.onboarding_PROD`
),
...
(
SELECT * FROM USERS 
WHERE ID NOT IN (SELECT DISTINCT ID FROM NEW_OB_TABLE)
),

NEW_OB_USERS AS
(
SELECT * FROM NEW_OB_TABLE
LEFT JOIN USERS USING(ID)    
 ...
 -->>>>>FIRST CASHIN <<<<<<< 
  usuarios_ci as (
    ...
    JOIN OLD_OB_USERS us ON us.id = u.uuid
    WHERE
      m.estado    = 'PROCESS_OK' 
      AND indnorcor  = 0 
