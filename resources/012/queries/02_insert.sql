DROP TABLE IF EXISTS `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Activity_Aux`;
CREATE TABLE `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Activity_Aux` AS (
WITH
  economics_app AS (
      ...
      FROM(
            ...
            LEFT JOIN `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Params` ON 1=1
            WHERE 
...
-- Tabla de resultados
CREATE TABLE IF NOT EXISTS `${project_target}.productos_tenpo.tenencia_camada_mes`
PARTITION BY Fecha_Analisis
CLUSTER BY user
AS (
    SELECT *
    FROM `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Activity_Aux`
    LIMIT 0
);

-- INSERT IDEMPOTENTE
DELETE FROM `${project_target}.productos_tenpo.tenencia_camada_mes`
WHERE Fecha_Analisis = (SELECT MAX(Fecha_Analisis) FROM `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Activity_Aux`);

INSERT INTO `${project_target}.productos_tenpo.tenencia_camada_mes`
SELECT * FROM `${project_target}.temp.TPCM_{{ds_nodash}}_0003_Activity_Aux`;