--SELECT 1 AS numero_4

---- DETALLE DE MAU MASTERCARD

DROP TABLE IF EXISTS `${project_target}.temp.query_mau_mastercard`;
CREATE TABLE `${project_target}.temp.query_mau_mastercard` AS (
...
SELECT
    ...
    user,
    max(linea like '%mastercard%' and nombre not like '%Devol%') f_mau
...
--- RESUMEN MAU MASTERCARD
DROP TABLE IF EXISTS `${project_target}.temp.query_mau_mastercard_summary`;
CREATE TABLE `${project_target}.temp.query_mau_mastercard_summary` AS (
...
SELECT
    DATE_TRUNC(fecha, MONTH) as mes,
    user,
    max(linea like '%mastercard%') f_mau
FROM `${project_source}.economics.economics`
GROUP BY 1,2
ORDER BY 1 DESC
), 
...
DROP TABLE IF EXISTS `${project_target}.temp.query_mau_mastercard_apertura`;
CREATE TABLE `${project_target}.temp.query_mau_mastercard_apertura` AS (
...
with compras as (
    ...
    where linea like '%mastercard%' and nombre not like '%Devolución%'
...
from `${project_target}.temp.query_mau_mastercard` 
...
FROM `${project_target}.temp.query_mau_mastercard_apertura`
GROUP BY 1,2
...
with data as (select
    *,
    case when mau_type in ('mau 1M','mau 2M','mau 3M','mau 4M','mau 5M') then 'MAU M1-M5' else mau_type end as apertura_mau
from `${project_target}.temp.query_mau_mastercard_summary`
