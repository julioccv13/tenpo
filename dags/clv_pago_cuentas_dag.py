...
with airflow.DAG(
    ...
    branching = BranchPythonOperator(
        ...
        templates_dict={
            ...
            "repo_name": "clv-pago-cuentas",
        },
    )

    data_ingestion = BashOperator(
        task_id="data_ingestion",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-data-ingestion-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                --set-env-vars PROCESS="ingesta_datos" \
                    --set-env-vars DS_NODASH="{{ds_nodash}}" \
                        --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                    && gcloud beta run jobs execute clv-pago-cuentas-data-ingestion-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_validation = BashOperator(
        task_id="data_validation",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-data-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-pago-cuentas-data-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation = BashOperator(
        task_id="data_preparation",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-data-preparation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-pago-cuentas-data-preparation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_training = BashOperator(
        task_id="model_training",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-model-training-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
            --set-env-vars PROCESS="entrenamiento" \
                ...
                && gcloud beta run jobs execute clv-pago-cuentas-model-training-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_validation = BashOperator(
        task_id="model_validation",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-model-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-pago-cuentas-model-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation_inference = BashOperator(
        task_id="data_preparation_inference",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-data-prep-infe-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-pago-cuentas-data-prep-infe-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    inference = BashOperator(
        task_id="inference",
        bash_command='gcloud beta run jobs create clv-pago-cuentas-inference-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-pago-cuentas \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-pago-cuentas" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-pago-cuentas-inference-{{ds_nodash}} \
                        --wait --region us-east1',
    )
