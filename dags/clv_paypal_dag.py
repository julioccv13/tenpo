...
with airflow.DAG(
    ...
    branching = BranchPythonOperator(
        ...
        templates_dict={
            ...
            "repo_name": "clv-paypal",
        },
    )

    data_ingestion = BashOperator(
        task_id="data_ingestion",
        bash_command='gcloud beta run jobs create clv-paypal-data-ingestion-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                --set-env-vars PROCESS="ingesta_datos" \
                    --set-env-vars DS_NODASH="{{ds_nodash}}" \
                        --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                    && gcloud beta run jobs execute clv-paypal-data-ingestion-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_validation = BashOperator(
        task_id="data_validation",
        bash_command='gcloud beta run jobs create clv-paypal-data-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-paypal-data-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation = BashOperator(
        task_id="data_preparation",
        bash_command='gcloud beta run jobs create clv-paypal-data-preparation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-paypal-data-preparation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_training = BashOperator(
        task_id="model_training",
        bash_command='gcloud beta run jobs create clv-paypal-model-training-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
            --set-env-vars PROCESS="entrenamiento" \
                ...
                && gcloud beta run jobs execute clv-paypal-model-training-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_validation = BashOperator(
        task_id="model_validation",
        bash_command='gcloud beta run jobs create clv-paypal-model-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-paypal-model-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation_inference = BashOperator(
        task_id="data_preparation_inference",
        bash_command='gcloud beta run jobs create clv-paypal-data-prep-infe-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-paypal-data-prep-infe-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    inference = BashOperator(
        task_id="inference",
        bash_command='gcloud beta run jobs create clv-paypal-inference-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-paypal \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-paypal" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-paypal-inference-{{ds_nodash}} \
                        --wait --region us-east1',
    )
