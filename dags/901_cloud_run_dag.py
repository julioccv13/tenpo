import json
from datetime import datetime, timedelta
from typing import Dict

import airflow
import pendulum
import requests
from airflow.decorators import task
from airflow.models import Variable
from airflow.operators import bash
from airflow.providers.http.operators.http import SimpleHttpOperator
from airflow.utils.task_group import TaskGroup
from plugins.slack import get_task_success_slack_alert_callback

environment = Variable.get("environment")
cloudrun_url = Variable.get("cloudrun_url")
sendgrid_email_from = "jarvis@tenpo.cl"
sendgrid_api_key = Variable.get("sendgrid_api_key")
project_target = Variable.get("project4")
project_source_1 = Variable.get("project3")
project_source_2 = Variable.get("project2")
SLACK_CONN_ID = f"slack_conn-{environment}"

data = {
    "main_python_file_uri": f"gs://{environment}-source-files/901/main.py",
    "python_file_uris": f"gs://{environment}-source-files/core.zip",
    "args": {
        "class_name": "Process901",
        "ts_nodash": '{{ dag_run.conf["exec_date_ts_nodash"] if "exec_date_ts_nodash" in dag_run.conf else ts_nodash }}',
        "ts": '{{ dag_run.conf["exec_date_ts"] if "exec_date_ts" in dag_run.conf else ts }}',
        "bucket_name": f"{environment}-source-files",
        "sendgrid_api_key": sendgrid_api_key,
        "sendgrid_email_from": sendgrid_email_from,
        "project_id": project_target,
        "project_source_1": project_source_1,
        "project_source_2": project_source_2,
        "project_target": project_target,
    },
}


default_args = {
    "owner": "Jarvis",
    "depends_on_past": False,
    "start_date": pendulum.datetime(2022, 7, 4, tz="UTC"),
    "email": ["jarvis@tenpo.cl"],
    "email_on_failure": True,
    "email_on_retry": False,
    "retries": 0,
    "retry_delay": timedelta(minutes=5),
    "on_failure_callback": get_task_success_slack_alert_callback(SLACK_CONN_ID),
}


with airflow.DAG(
    "901_cloudrun_dof",
    catchup=False,
    default_args=default_args,
    schedule_interval="0 */2 * * *",
) as dag:

    @task
    def launch_cloud_run(data: Dict, **kwargs):
        response = requests.post(cloudrun_url, json=data)
        if "Success" in response.text and response.status_code == 200:
            return response.text
        else:
            raise RuntimeError(response.text)

    compilar = launch_cloud_run(data)

    compilar
