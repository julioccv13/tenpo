...
with airflow.DAG(
    ...
    data_ingestion = BashOperator(
        ...
        bash_command='gcloud beta run jobs create clv-tyba-data-ingestion-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                --set-env-vars PROCESS="ingesta_datos" \
                    --set-env-vars DS_NODASH="{{ds_nodash}}" \
                        --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                    && gcloud beta run jobs execute clv-tyba-data-ingestion-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_validation = BashOperator(
        task_id="data_validation",
        bash_command='gcloud beta run jobs create clv-tyba-data-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-tyba-data-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation = BashOperator(
        task_id="data_preparation",
        bash_command='gcloud beta run jobs create clv-tyba-data-preparation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-tyba-data-preparation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_training = BashOperator(
        task_id="model_training",
        bash_command='gcloud beta run jobs create clv-tyba-model-training-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
            --set-env-vars PROCESS="entrenamiento" \
                ...
                && gcloud beta run jobs execute clv-tyba-model-training-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    model_validation = BashOperator(
        task_id="model_validation",
        bash_command='gcloud beta run jobs create clv-tyba-model-validation-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-tyba-model-validation-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    data_preparation_inference = BashOperator(
        task_id="data_preparation_inference",
        bash_command='gcloud beta run jobs create clv-tyba-data-prep-infe-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-tyba-data-prep-infe-{{ds_nodash}} \
                        --wait --region us-east1',
    )

    inference = BashOperator(
        task_id="inference",
        bash_command='gcloud beta run jobs create clv-tyba-inference-{{ds_nodash}} \
        --image gcr.io/{{ project_target }}/clv-tyba \
            --tasks 1 \
                ...
                --set-env-vars DS_NODASH="{{ds_nodash}}" \
                    --set-env-vars DS="{{ds}}" \
                            --set-env-vars  PROJECT_TARGET="{{ project_target }}" \
                                --set-env-vars  REPO_NAME="clv-tyba" \
                                    --region us-east1 --memory=8192M  --cpu=4 \
                && gcloud beta run jobs execute clv-tyba-inference-{{ds_nodash}} \
                        --wait --region us-east1',
    )
